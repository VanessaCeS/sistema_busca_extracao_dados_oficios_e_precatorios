...
class ArcherInstance:
    ...
    def from_application(self, app_name=None):
        ...
        if not self.current_application or self.current_application != app_name and self.current_application:
            try:
                ...
                all_folders = []
                application_id = None
                self.all_applications_names = []
                for app in data:
                    self.all_applications_names.append(app["RequestedObject"]["Name"])
    ...
    def get_field_options(self, field_name):
        ...
        try:
            ...
            for value_id in data:
                self.fields[field_name][value_id["RequestedObject"]["Name"]] = value_id["RequestedObject"]["Id"]
    ...
    def create_sub_record(self, fields_json, subform_name, record_id=None):
        ...
        if record_id:
            post_header["X-Http-Method-Override"] = "PUT"
            body = json.dumps(
                {"Content": {"Id": record_id, "LevelId": subform_level_id, "FieldContents": transformed_json},
    ...
    def get_level_valueslist(self):
        ...
        response = requests.post(url, headers=dict(self.header), data="").json()
        fields_list = {}
        for item in response:
            fields_list[item["RequestedObject"]["Name"]] = item["RequestedObject"]["Id"]
        return fields_list
