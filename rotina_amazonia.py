...
from utils import apagar_arquivos, encontrar_indice_linha, limpar_dados, mandar_dados_regex, mandar_documento_para_ocr, regex, tipo_precatorio
...
def ler_xml():   
  ...
  for d in dados:
        ...
        if verificar_tribunal(d['processo']) and d['processo_origem'] != '':
          ...
          pass
  apagar_arquivos(['./arquivos_txt_amazonas', './arquivos_pdf_amazonas', './fotos_oab', './arquivos_texto_ocr'])
...
def extrair_dados_pdf(arquivo_pdf, dados_xml, arquivo_txt):
    ...
    indice_credor = encontrar_indice_linha(linhas, "credor") + 1
    ...
    advogado = mandar_dados_advogado(linhas, dados_xml['processo_geral'], indice_procurador, indice_advogado)
    natureza = cortar_natureza(arquivo_txt)
    credor_e_documento = extrair_credor_e_documento(linhas[indice_credor])
    valor_principal_e_juros = extrair_valor_principal_e_juros(linhas[indice_valor_juros])
    ...
    if dados_regex['valor_global'] == '' and dados_regex['valor_principal'] != '' and valor_principal_e_juros['valor_juros'] != '':
      valores = float(dados['valor_principal'].replace('.', '').replace(',', '.')) + float(valor_principal_e_juros['valor_juros'].replace('.', '').replace(',', '.'))
      dados_regex['valor_global'] = str(valores)

    dados = dados | dados_xml |  credor_e_documento | valor_principal_e_juros | advogado | natureza | dados_regex
