...
def extrair_dados_pdf(arquivo_pdf, arquivo_txt, dados_pdf):
  ...
  if indice_credor != None or indice_autor != None:
    ...
    texto_ocr = mandar_documento_para_ocr(pdf_para_img, '4', dados_pdf["processo_geral"] , 'arquivos_txt_mato_grosso_do_sul')

    with open(texto_ocr, 'r', encoding='utf-8') as f:
      linhas = f.readlines()
    ...
    if indice_advogado != None:
      ...
      if advogado.upper().strip() != 'NULL':
        ...
        dados_advogado = login_cna(dados['oab'], dados['seccional'],  '',dados['advogado'], dados_pdf['processo'])
